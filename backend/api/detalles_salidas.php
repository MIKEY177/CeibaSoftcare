...
// ── GET: listar detalles de una salida ──────────────────────────────────────
if ($method === 'GET') {
    ...
    if ($id_salida === 0) {
        ...
        echo json_encode([
            "success" => false,
            "errores" => ["general" => "❗Se requiere id_salida."]
    ...
    $stmt = mysqli_prepare($conn,
        "SELECT d.id_detalle_salida,
         ...
         ORDER BY d.id_detalle_salida ASC"
    );
...
function productoExiste(mysqli $conn, int $id): bool
{
 ...
 if ($method === 'POST') {
    $id_salida1           = intval($body['id_salida1'] ?? 0);
    ...
    mysqli_stmt_bind_param($stmt, "iiids",
        $id_salida1, $id_producto1, $cantidad_presentacion,
        $cantidad_total, $motivo
    );
...
// ── PUT: editar detalle ──────────────────────────────────────────────────────
if ($method === 'PUT') {
    $id_detalle            = intval($body['id_detalle_salida']   ?? 0);
    ...
    if ($id_detalle === 0)
        $errores['id_detalle_salida'] = "❗ID de detalle inválido.";
    ...
    $stmtDetActual = mysqli_prepare($conn,
        "SELECT cantidad_total, id_producto1
         FROM detalles_salidas_pro
         WHERE id_detalle_salida = ? AND activo = 1"
    );
    mysqli_stmt_bind_param($stmtDetActual, "i", $id_detalle);
    mysqli_stmt_execute($stmtDetActual);
    $resDetActual = mysqli_stmt_get_result($stmtDetActual);
    $detalleActual = mysqli_fetch_assoc($resDetActual);
    mysqli_stmt_close($stmtDetActual);
    ...
    if (empty($errores)) {
        ...
        if (mysqli_stmt_num_rows($s) === 0) {
            $errores['id_producto1'] = "❗No se encontró stock para este producto.";
        } else {
            ...
            if (floatval($cantidad_total) > $maxStock) {
                $errores['cantidad_total'] = "❗La cantidad total no puede exceder el stock disponible ($maxStock).";
            }
        }
        mysqli_stmt_close($s);
    }
    ...
    mysqli_stmt_bind_param($stmt, "iidsi",
        $id_producto, $cp, $ct, $motivo, $id_detalle
    );
...
if ($method === 'DELETE') {
    ...
    $stmt = mysqli_prepare($conn,
        "UPDATE detalles_salidas_pro SET activo = 0 WHERE id_detalle_salida = ?"
    );
